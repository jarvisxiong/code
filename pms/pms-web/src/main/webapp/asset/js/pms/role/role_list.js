function gen_action(item) {
	...
	var objAction = {
		edit: [
			{
				label: '编辑',
				href: "javascript: iframeFullPage('"+ rootPath + '/role/form.do?id=' + id +"')"
			}
		],
		delete: [
			{
				label: '删除',
				href: rootPath + '/role/delete.do?id=' + id
			}
		],
		roleUser: [
			{
				label: '分配',
				href: "javascript: iframeFullPage('"+ rootPath + '/role/roleUserList.do?id=' + id +"')"
			}
		]
	};
