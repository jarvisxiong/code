...
public class ExcelCallForNewUserActivity extends ActivityCommodityServiceImpl implements Callable<String> {
	...
	public ExcelCallForNewUserActivity(int i, String[] data, ActivityManager activityManager, ActivityTypeEnum enum1,
			ActivityCommoditySkuService ctivityCommoditySkuService, CommoditySkuApiConsumer commoditySkuApiConsumer,
			ActivityCommoditySkuService activityCommoditySkuService) {
		this.i = i;
		this.data = data;
	...
	public String call() throws Exception {
		...
		try {
			...
			++col;
			// 标题仅仅验证null
			ExcelImportConstant.notisNullvalidate(ExcelImportConstant.trimString(data[col]), ExcelImportConstant.titleString, row, col,
					errorVaildate);
			++col;
			// 置顶序号
			...
			++col;
			// 限购数量
			Integer limitCount = ExcelImportConstant.limitCountValidate(ExcelImportConstant.trimString(data[col]),
					ExcelImportConstant.limitCountString, row, col, errorVaildate);
			++col;
			...
			getskuTransformList(data[0].trim(), commoditysku.getCommodity().getCode(), limitCount, row, col); // 验证限定数量必须小于用户可购买量
			...
			activityCommdity.setIdLimitCount(1);// 用户限购量默认为1
	...
	public void  getskuTransformList(String skubarcode,String code,Integer limitCount,int row,int col) throws Exception{
		...
		if(skuTransformList==null || skuTransformList.size()==0){
			errorVaildate.append(alterString(ExcelImportConstant.tiaoxingmaString, row, col, skubarcode+"商品匹配不到任何商品信息"));
			throw new  RuntimeException(alterString(ExcelImportConstant.tiaoxingmaString, row, (col-1), skubarcode+",商品匹配不到任何商品信息"));
		}
		...
		if(StringUtil.isEmpty(commoditySkuTransform.getUserCanBuy())){
			...
			errorVaildate.append(alterString(ExcelImportConstant.tiaoxingmaString, row, col, skubarcode+"商品找不到库存"));
			throw new  RuntimeException(alterString(ExcelImportConstant.tiaoxingmaString, row, (col-1), skubarcode+"商品找不到库存"));
		}
		if(limitCount>Integer.parseInt(commoditySkuTransform.getUserCanBuy())){
			errorVaildate.append(alterString(ExcelImportConstant.tiaoxingmaString, row, (col-1), skubarcode+"商品的库存"+commoditySkuTransform.getUserCanBuy()+"小于"+
			"商品限制数量"+limitCount));
			throw new  RuntimeException(alterString(ExcelImportConstant.tiaoxingmaString, row, (col-1), skubarcode+"商品的库存"+commoditySkuTransform.getUserCanBuy()+"小于"+
			"商品限制数量"+limitCount));
		}
