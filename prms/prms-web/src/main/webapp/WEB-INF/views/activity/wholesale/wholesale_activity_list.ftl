...
<div class="tab-content">
	<div class="tab-pane fade in active" id="myAccount">
		<div class="col-md-12">
			<div class="search">
				<form id="find-page-orderby-form" action="${BasePath !}/wholesaleActivity/list.do" method="post">
					...
					<div class="btn-div3">
						...
						<@permission name="prms:wholesale:add">
						<a href="${BasePath !}/wholesaleActivity/form.do"  class="btn btn-primary btn-sm" ><i class="fa fa-plus"></i>&nbsp;&nbsp;添加</a>
						</@permission>
					</div>
				</form>
			</div>
		...
		<table class="table table-hover table-striped bor2 table-common">
			...
			<tbody>
				...
				<tr id="${(item.id) !}" pId="${(item.pId) !}" ondblclick="view('${(item.id) !}')">
					...
					<td align="center"><a href="${BasePath !}/wholesaleActivity/view.do?id=${(item.id) !}">${(item.activityNo) !}</a></td>
					...
					<td>
						...
						<#if item.releaseStatus == '0'>
							<@permission name="prms:wholesale:release">
							   发布
						   </@permission>
						   <#else>
						   <@permission name="prms:wholesale:revoke">
						            撤销
						   </@permission>
						 </#if></a> 
						<#if item.releaseStatus ??><#if (item.releaseStatus) = '0'>
						    <@permission name="prms:panicbuy:edit">
							| <a href="${BasePath !}/wholesaleActivity/form.do?id=${(item.id) !}">编辑</a>
							</@permission>
							<@permission name="prms:panicbuy:goodsconfig">
							| <a href="javascript:ordinaryCommodity('${(item.id) !}');">商品设置</a>
							</@permission>
							<@permission name="prms:wholesale:delete">
							| <a href="javascript:deleteItem('${(item.id) !}')">删除</a>
							</@permission>
						<#else>
						    <@permission name="prms:wholesale:change">
							| <a href="${BasePath !}/wholesaleActivity/form.do?id=${(item.id) !}">特改</a>
							</@permission>
							<@permission name="prms:panicbuy:goodsconfig">
...
<script type="text/javascript" src="${BasePath !}/asset/js/activity/wholesale/wholesale_activity_list.js?v=${ver !}"></script>
<script type="text/javascript">
	 function onEmpty() {
         location.href="${BasePath !}/wholesaleActivity/list.do";            
     }
	//查看
	function view(id) {
		window.location.href = '${BasePath !}/wholesaleActivity/view.do?id=' + id;
	}
	</script>
</body>
</html>