...
<div class="tab-content">
	<div class="tab-pane fade in active" id="myAccount">
		<div class="col-md-12">
			<div class="search">
				<form id="find-page-orderby-form" action="${BasePath !}/panicbuyActivity/dataList.do" method="post">
					...
					<div class="inquire-ul">
						<div class="form-tr">
							...
							<div class="form-td">
								<label>推荐状态：</label> 
								<div class="div-form">
									<select class="form-control input-sm txt_mid input-select" id="appRecommend" name="appRecommend">
										<option value="">不限</option>
										<option value="0" <#if (activity.appRecommend) ??><#if activity.appRecommend == '0'>selected="selected"</#if></#if> >未推荐</option>
										<option value="1" <#if (activity.appRecommend) ??><#if activity.appRecommend == '1'>selected="selected"</#if></#if> >已推荐</option>
									</select>
								</div>
							</div>
							<div class="form-td">
					...
					<div class="btn-div3">
						...
						<a href="${BasePath !}/panicbuyActivity/dataForm.do"  class="btn btn-primary btn-sm" ><i class="fa fa-plus"></i>&nbsp;&nbsp;添加</a>
						 </@permission>
						<a href="javascript:appRecomendManager()" class="btn btn-primary btn-sm" ><i class="fa fa-plus"></i>&nbsp;&nbsp;APP推荐管理</a>
 					</div>
				</form>
			</div>
		</div>
		...
		<table class="table table-hover table-striped bor2 table-common">
			<thead>
				<tr>
					...
					<th class="tab-td-center">推荐状态</th>
			...
			<tbody>
				...
				<tr ondblclick="view('${(item.id) !}')">
					...
					<td align="center"><#if (item.appRecommend)??><#if item.appRecommend == '0'>未推荐<#else>已推荐</#if></#if></td>
					...
					<td>
						<#if (item.releaseStatus)??>
					     ...
					     <@permission name="prms:panicbuy:fabu">
						  ...
						  <#else>
						  <@permission name="prms:panicbuy:revoke">
						   <a href="javascript:isEnabled('${(item.id) !}',<#if (item.releaseStatus)??><#if item.releaseStatus == '0'>1<#else>0</#if></#if>) ">
								撤销
							</a>
						</@permission>
						...
						| <a href="${BasePath !}/panicbuyActivity/dataForm.do?id=${(item.id) !}">编辑</a>
						</@permission>
						...
						<@permission name="prms:panicbuy:change">
						| <a href="${BasePath !}/panicbuyActivity/dataForm.do?id=${(item.id) !}">特改</a>
						</@permission>
...
<#include "../common/select.ftl" encoding="utf-8">
<script type="text/javascript" src="${BasePath !}/asset/js/activity/panicbuyactivity_list.js?v=${ver !}"></script>
</body>
</html>