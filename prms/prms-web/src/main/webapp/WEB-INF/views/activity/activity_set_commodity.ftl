...
<div class="row">
	...
	<div class="row">
	    <div class="col-lg-10 col-md-12 col-sm-12">
	        <form id="myform" action="${BasePath !}/presellActivity/saveCommodity.do" method="post">
	           ...
	           <ul></ul>
	            ...
	            <div class="form-tr">
						<div class="form-td">
							<label><i>*</i>开始时间：</label>
								...
								onfocus="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm:ss'})" value="${(activityCommodity.startDate?string('yyyy-MM-dd HH:mm:ss')) !}">  																	
								</div>
						</div>
						<div class="form-td">
							<label><i>*</i>结束时间：</label>
								...
								onfocus="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm:ss'})" value="${(activityCommodity.endDate?string('yyyy-MM-dd HH:mm:ss')) !}"> 									
								</div>
						</div>
	              </div>
	             <div class="form-tr">
						<div class="form-td">
							<label><i>*</i>发货时间：</label>
								<div class="div-form"> 
									 	<input name="deliverDateStr" id="deliverDateStr" class="form-control txt_mid input-sm" data-rule-required="true"  data-msg-required="发货时间不能为空"
								onfocus="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm:ss'})" value="${(activityCommodity.deliverDate?string('yyyy-MM-dd HH:mm:ss')) !}">  																	
								</div>
						</div>
						<div class="form-td">
							<label></label>
	            ...
	            <div class="form-tr">
					...
					<div class="form-td">
							<label>ID限购量：</label>
							<div class="div-form"> 
								<input type="text" name="idLimitCount"  onkeyup="onlyInputNum(this);" id="idLimitCount" value="${(activityCommodity.idLimitCount) !}">
							</div>
					</div>
	               </div>
		 <div class="form-tr">
					<div class="form-td">
						...
						<div class="form-td">
							<label>销量增量值：</label>
							<div class="div-form"> 
								<input type="text" name="saleIncrease"  id="saleIncrease"  onkeyup="onlyInputNum(this);" value="${(activityCommodity.saleIncrease) !}">
							</div>
						</div>
	              </div>
	             <div class="form-tr">
	                   <div class="form-td">
							<label></label>
								<div class="div-form"> 
										启用特殊增量<input type="checkbox" name="enableSpecialCount"  id="enableSpecialCount" value="${(activityCommodity.enableSpecialCount) !}" onclick="radioAmount();" <#if (activityCommodity.enableSpecialCount) ??><#if activityCommodity.enableSpecialCount=='1'>checked="checked" </#if></#if> />           							
								</div>
						</div>
						<div class="form-td">
							<label>特殊增量值：</label>
	            ...
	            <div class="form-tr">
	                   <div class="form-td">
							<label><i>*</i>所属标签：</label>
								<div class="div-form"> 
										<select class="form-control input-sm txt_mid input-select" id="preSaleTag.id" name="preSaleTag.id" data-rule-required="true"  data-msg-required="预售标签不能为空">
                                            <option value="">请选择标签 </option>   
                                            <#if tagList?? >
                                            	<#list tagList as item >
                                            		<option value="${(item.id) !}" 
                                            		<#if  activityCommodity?? && activityCommodity.preSaleTag?? >
                                            			<#if activityCommodity.preSaleTag.id==item.id >	
                                            				selected="selected"                                       
                                            			</#if></#if>>${(item.name) !}</option>                                           	
                                            	</#list> 
											</#if>		     											
										 </select>		
								</div>
							...
							<table class="table table-hover table-striped bor2 table-common" id="tableValue" >
								<thead>
									<tr>
											...
											<th width="8%">商品条码</th>
											<th width="8%">优惠价/元</th>
											<th width="8%">活动优惠价/元</th>
											<th width="8%">用户可购买数量</th>
											<th width="8%">限定数量</th>							
									</tr>
								</thead>
								<tbody id="dateTr">
								<#if activityCommoditySkuList?? > 
									<#list activityCommoditySkuList as item >
										<tr>									
											...
											<td width="8%"><input  type="text" name="limitNum${item_index + 1}" class="limitNum" onkeyup="onlyInputNum(this);" style="width:80px;" onBlur="caclTotalNum()" value="${(item.limitCount) !}" data-rule-required="true"  data-msg-required="SKU限定数量不能为空"  data-rule-isIntGteZero="true"  data-msg-isIntGteZero="SKU限定数量为大于等于0的整数" ></td>		
										</tr>			
									</#list> 
								</#if>								
								</tbody>
...
<script type="text/javascript" src="${BasePath !}/asset/js/activity/activity_set_commodity.js?v=${ver !}"></script>
...
var image_path = "${image_path}";
    ...
    function  changeexecuteValidateFrom(){
