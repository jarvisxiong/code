...
<div class="tab-content">
	<div class="tab-pane fade in active" id="myAccount">
		<div class="col-md-12">
			<div class="search">
				<form id="find-page-orderby-form" name="myform" action="${BasePath !}/wholesaleActivity/commoditylist.do" method="post">
					...
					<div class="btn-div3">
                        ...
                        <a id="table-deleteBtn" class="btn btn-primary btn-sm" table_action="wholesaleActivity/commodityDeletes.do"><i class="fa fa-remove"></i>&nbsp;&nbsp;批量删除</a>
                        <a href="#" onClick='showMember("${activityCommodity.activity.id}","WHOLESALE_MANAGER")' class="btn btn-primary btn-sm" ><i class="fa fa-plus" ></i>&nbsp;&nbsp;批量导入</a>
                    </div>
				</form>
			</div>
		</div>
			...
			<tbody>
				...
				<tr id="${(item.id) !}" ondblclick="view('${(item.activity.id) !}','${(item.activity.activityNo) !}','${(item.id) !}','view')">
					...
					<td align="center">
						...
						<a name="delete_item" href="${BasePath !}/wholesaleActivity/commodityDelete.do?id=${(item.id) !}">删除</a>
					</td>
				</tr>
				</#list>
				</#if>
...
<script type="text/javascript" src="${BasePath !}/asset/js/activity/wholesale/wholesale_commodity_list.js?v=${ver !}"></script>
</body>
</html>