...
<body>
	...
	<div class="tab-content">
		<div class="tab-pane fade in active" id="memberList">
			<div class="col-md-12">
				<div class="search">
					<form id="find-page-orderby-form" action="${BasePath !}/activityGive/findzpCommodity.do" method="post">
					
					<input  id="find-page-index-noidList"  name="noidList" type="hidden" value="${noidList!""}"  />
						<input id="find-page-index" type="hidden" name="pageIndex" value="1" /> 
						<input id="find-page-count" type="hidden" value="${(pageObj.pageCount) !}" />
						<input id="find-page-size" type="hidden" name="pageSize" value="${(pageObj.pageSize) !}" />
						<input id="giveCommodityId" name="giveCommodityId" type="hidden"  value="${(giveCommodityId) !}">
			...
			<table class="table table-hover table-striped bor2 table-common">
				...
				<tbody>
					...
					value="${(item.id) !},${(item.code) !}" name="check"></td>
						<td>${item_index + 1}</td>
						<td>${(item.name) !}</td>
						<td>${(item.code) !}</td>
	...
	<script type="text/javascript" src="${BasePath !}/asset/js/activityGive/findzpCommodity.js?v=${ver !}"></script>
