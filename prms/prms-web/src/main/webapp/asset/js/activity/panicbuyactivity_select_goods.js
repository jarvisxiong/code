...
function getUpdata(commodityId,commName,commBarCode,commPrice,code,buyType) {
	if(buyType!='PANIC_BUY'&&buyType!='COMMON_BUY'){
		...
		//设置返回tr
		$.ajax({
			url : rootPath + "/panicbuyActivity/getCommoditySku.do",
			data : {
				commodityCode : code
			},
			type : "POST",
			...
			success : function(result) {
				...
				{				
					showStr+='<tr>'
							...
							+'<td><input  type="text" name="limitNum'+i+'"  class="limitNum" onkeyup="onlyInputNum(this);" style="width:80px;" onBlur="caclTotalNum()" data-rule-required="true"  data-msg-required="SKU限定数量不能为空"  data-rule-isIntGteZero="true"  data-msg-isIntGteZero="SKU限定数量为大于等于0的整数" ></td>'
							+'</tr>';
				}
			}
		});	
