...
function getUpdata(commodityId,commName,commBarCode,commPrice,code,buyType) {
	if(buyType!='PRE_SALE'&&buyType!='COMMON_BUY'){
		...
		//设置返回tr
		$.ajax({
			url : rootPath + "/presellActivity/getCommoditySku.do",
			data : {
				commodityCode : code
			},
			type : "POST",
			...
			success : function(result) {
				...
				{				
					showStr+='<tr>'
							...
							+'<td><input  type="text"  name="acprice'+i+'" class="acprice" onkeyup="onlyInputPrice(this);" skuId="'+result[i].id+'" skuprice="'+result[i].favourablePrice+'" skuCode="'+result[i].skuCode+'" style="width:80px;" onBlur="caclTotalprice()" data-rule-isFloatGtZero="true"   data-msg-isFloatGtZero="SKU优惠价必须大于0" data-rule-decimal="true"   data-msg-decimal="小数点不能超过两位数"  data-rule-required="true"  data-msg-required="SKU优惠价不能为空"    ></td>'
							+'<td><input  type="hidden" name="userCanBuy'+i+'"  class="userCanBuy" value="'+result[i].userCanBuy+'">'+result[i].userCanBuy+'</td>'
							+'<td><input  type="text" name="limitNum'+i+'"  class="limitNum" onkeyup="onlyInputNum(this);" style="width:80px;" onBlur="caclTotalNum()" data-rule-required="true"  data-msg-required="SKU限定数量不能为空"  data-rule-isIntGteZero="true"  data-msg-isIntGteZero="限定数量为大于等于0的整数"></td>'
							+'</tr>';
				}
			}
		});	
...
//		parent.changeexecuteValidateFrom();
