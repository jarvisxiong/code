...
function toSelectCommdity(){
		var url = rootPath + '/presellActivity/toCommdity.do';		
		var dlg=dialog({
...
function butsubmit(){
	...
	var deliverDateStr=$("#deliverDateStr").val();//活动商品发货时间
	...
	if(Date.parse(startDateStr.replace(/-/g, "/")) < Date.parse(activityStartDate.replace(/-/g, "/")) ||
		...
		$.frontEngineDialog.executeDialogOK('提示信息','开始时间必须在预售活动开始结束时间之内！','300px');
		return false;
	}
	if(Date.parse(endDateStr.replace(/-/g, "/")) > Date.parse(activityEndDate.replace(/-/g, "/")) ||
		...
		$.frontEngineDialog.executeDialogOK('提示信息','结束时间必须在预售活动开始结束时间之内！','300px');
		return false;
	}
	if(Date.parse(deliverDateStr.replace(/-/g, "/")) < Date.parse(startDateStr.replace(/-/g, "/"))){
		$.frontEngineDialog.executeDialogOK('提示信息','发货时间不能小于开始时间！','300px');
		return false;
	}
...
function toBack(){
	...
	window.location.href = rootPath + '/presellActivity/commoditylist.do?activityId='+actId+'&activityNo='+actNo;
}
