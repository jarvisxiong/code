if (typeof FFZX == 'undefined') {
		...
		$(window).on('resize', function(){
			...
			if ($iframeFullPage.length) {
				...
				var doResize = function(){				
					$iframeFullPage.height(parent.$('iframe').height());
				}
	...
	*/
		...
		if (getCurIframeName != '') {
			...
			if ($iframe.length) {
				...
				if (curIframeObj[1].indexOf(getCurUrl) == -1) {
					...
					if ($parentIframe.parent()[0].id && $parentIframe.parent()[0].id==='content:'+curIframeID){
	...
	function iframeFullPage(url){
		...
		var iframe = '<iframe frameborder="0" scrolling="auto" src="'+ url +'" name="' + winName + ','+ window.location.href + '" class="iframeFullPage" style="height:' + parent.$('iframe').height() + 'px">';
		$('body').append(iframe);
	}		
