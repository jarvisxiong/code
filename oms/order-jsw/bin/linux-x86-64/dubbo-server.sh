...
# Application
APP_NAME="order-jsw"
APP_LONG_NAME="order-jsw Dubbo Server "
...
launchinternal() {
    ...
    then 
        ...
        COMMAND_LINE="$CMDNICE \"$WRAPPER_CMD\" \"$WRAPPER_CONF\" wrapper.syslog.ident=\"$APP_NAME\" wrapper.pidfile=\"$PIDFILE\" wrapper.name=\"$APP_NAME\" wrapper.displayname=\"$APP_LONG_NAME\" wrapper.daemonize=TRUE $ANCHORPROP $IGNOREPROP $STATUSPROP $COMMANDPROP $LOCKPROP wrapper.script.version=3.5.16 $ADDITIONAL_PARA"
        eval $COMMAND_LINE
    else
...
console() {
    ...
    then
        ...
        # The string passed to eval must handles spaces in paths correctly.
        COMMAND_LINE="$CMDNICE \"$WRAPPER_CMD\" \"$WRAPPER_CONF\" wrapper.syslog.ident=\"$APP_NAME\" wrapper.pidfile=\"$PIDFILE\" wrapper.name=\"$APP_NAME\" wrapper.displayname=\"$APP_LONG_NAME\" $ANCHORPROP $STATUSPROP $COMMANDPROP $LOCKPROP wrapper.script.version=3.5.16 $ADDITIONAL_PARA"
        eval $COMMAND_LINE
    else
        eval echo `gettext '$APP_LONG_NAME is already running.'`
        exit 1
...
installUpstart() {
    ...
    if [ -f "${REALDIR}/${APP_NAME}.install" ] ; then 
        ...
        echo "env LANG=en_US.UTF-8" >> "/etc/init/${APP_NAME}.conf"
        ...
        if [ "$DIST_OS" = "solaris" ] ; then
            ...
            if [ -f "/etc/rc.d/$APP_NAME" ] ; then
                ...
                if [ -f "${REALDIR}/${APP_NAME}.install" ] ; then
                    ...
                    echo 'start_cmd="order-jsw_start"' >> "/etc/rc.d/$APP_NAME"
                    echo 'load_rc_config $name' >> "/etc/rc.d/$APP_NAME"
                    echo 'status_cmd="order-jsw_status"' >> "/etc/rc.d/$APP_NAME"
                    echo 'stop_cmd="order-jsw_stop"' >> "/etc/rc.d/$APP_NAME"
