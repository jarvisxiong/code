...
<div class="row">
       <div class="col-md-12">
         <div class="box-body">
     <#if applyID?exists>
    	 <#assign pageAnswer="http://${(orderDomain) !}:${(orderPort) !}/order-web/aftersaleapickup/toApprove.do?id=${(applyID) !}&pickupNo=${pickupNo! }" />
    	 <#assign action="${BasePath !}/aftersaleapickup/toApprove.do?id=${(applyID) !}&pickupNo=${pickupNo! }"/>
    	 <#else>
    	 <#assign pageAnswer="http://${(orderDomain) !}:${(orderPort) !}/order-web/aftersaleapickup/toApprove.do?pickupNo=${pickupNo! }" />
    	 <#assign action="${BasePath !}/aftersaleapickup/toApprove.do?pickupNo=${pickupNo! }"/>
     </#if>
<div class="tab-content">
        ...
        <div class="orderleft">
            <ul>
                ...
                <li>状态：<span class="txt">
               			 <#if pickup.pickupStatus?? && pickup.pickupStatus=="0">
								未审核
						<#elseif pickup.pickupStatus=="1">
								已通过
						<#else>
								已驳回
						</#if>	
            ...
            <div class="btn-div3">
            	<#if pickup.pickupStatus?? && pickup.pickupStatus=="0">	
            		<@permission name="oms:aftersale:printer">
						<a class="btn btn-primary" href="${BasePath !}/aftersaleapickup/toPrinter.do?id=${(apply.id) !}" >打印</a>
					</@permission>		           		 										
				</#if>	               
                ...
                <#if pageType?exists && pageType=='AftersalePickup'>
               		<button class="btn btn-primary btn-go-back" type="submit" onclick="isReturn();">返回</button>
               		<#else>
               		<button class="btn btn-primary" type="submit" onclick="history.go(-3);">返回</button>
                </#if>
            </div>
        </div>
    </div>
    <div class="orderline">                                                         
        ...
        <div class="addpege">
            <ul>
                <li>关联订单：<a href="${BasePath !}/order/view.do?id=${(apply.order.id) !}&pageType=${(action)!}">${(pickup.orderNo) !}</a></li>
<!--                 <li>收货仓库：${(apply.order.storageName) !}</li>重新定义为县仓 -->
                ...
                <li>关联售后申请单号：<a href="${BasePath !}/aftersaleapply/toApprove.do?id=${(apply.id) !}">${(pickup.applyNo) !}</a></li>
    ...
    <div class="tab-content mgb10">
        <div class="tab-pane fade in active" id="myAccount">
            ...
            <div class="check-description mgb20" >
                <p>审核描述：</p>
                <textarea disabled="disabled" class="form-control" rows="3"  id="serviceApproveDesc" name="serviceApproveDesc">${(apply.serviceApproveDesc) !}</textarea>
            </div>
