...
<body>
	...
	<div class="tab-content">
		<div class="tab-pane fade in active" id="brandList">
			<div class="col-md-12">
				<div class="search">
					<form  action="${BasePath !}/purchaseArea/list.do" id="myform" name="myform" method="post">
						...
						<div class="inquire-ul">
							<div class="form-tr"><br/>
								...
								<div class="form-td">
									...
									<div class="div-form wMonospaced">   
										<label style="width:auto;">
										<input type="checkbox" name="codeIs" id="codeIs" value="0" <#if (codeIs) ?? && codeIs == "0">checked="checked"</#if>
										/>购买区域为空</label>
									</div>
								</div>		
							</div>
						...
						<div class="btn-div3">
							...
							<a href="${BasePath !}/purchaseArea/list.do?tblshow=1" class="btn btn-primary btn-sm"><i class="fa fa-remove"></i>&nbsp;&nbsp;清空</a>
				...
				<table class="table table-hover table-striped bor2 table-common">
					...
					<tbody>
						...
						<tr ondblclick="view('${(item.id) !}','${(item.code) !}')">
							...
							<!-- <td class="tab-td-center">
								<#if (item.warehouse)?? && item.warehouse == "0">是<#else>否</#if>
							</td>   -->
							<td class="tab-td-center">
									${(item.addressName) !}
							</td>
							<td class="tab-td-center">
								<#if item.stockLimit == "1" &&  item.areaCategory == "0">
									<@permission name="order:purchaseArea:setArea">
										<a href="${BasePath !}/purchaseArea/onesetsite.do?commodityCode=${(item.code) !}" class="btn btn-primary btn-sm">设置购买区域</a>
									</@permission>
								<#else>
