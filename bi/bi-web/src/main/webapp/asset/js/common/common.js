...
$(function(){
	...
	*/
	...
	//$(document).on('click', '.table th :checkbox', function() {
		//var $table = $(this).closest('.dataTables_wrapper') || $(this).closest('.table');
	...
	.on('mouseenter', 'input[value="返回"]', function(){
	...
	.on('click', 'input[value="返回"]', function(){	
		...
		if (/location/.test(strOnclick)) {
			var href = $.trim(strOnclick.split('=')[1]);
...
$(document).on('click', '.dataTables_wrapper tbody .action_delete', function (e) {
	...
	var url = this.href;
	var dataTableId = $this.closest('.dataTables_wrapper').attr('id').replace('ff_DataTable_', '').replace('_wrapper', '');
...
function reloadData(dataTableId){
	...
	// 保留在当前页刷新
	FFZX.DT['ff_DataTable_'+ dataTableId].ajax.reload(null, false);
}

function callbackIframeFullpage(dataTableId){
	...
	genHtml: function(){
		...
		if ($.isArray(this.permission) && this.permission.length) {
			...
			$.each(this.permission, function(idx, ele){	
				...
				if ('param' in dataItem) {
					
					$.each(_data[ele]['param'], function(key, val){						
				...
				arrAction.push('<a href="'+ dataItem.href + strParam + '" class="action_'+ ele +'">'+ dataItem.label +'</a>');				
			});			
...
FFZX.initDataTable = function(obj) {
	...
	var dataTableId = 'ff_DataTable_' + obj.div_id;
	var genThead = function(obj){			
		var arr = [];
		
		$.each(obj, function(idx, ele){
			arr.push('<th class="clm_'+ ele.data +'">'+ ele.label +'</th>');
		});
		
		return arr.join('');
	};
	
	var strTable = '<table id="ff_DataTable_'+ obj.div_id +'" class="table table-hover table-striped table-bordered">'
		+ 	'<thead>'
		+		'<tr>'					
		+			'<th class="clm_index"></th>'
		+			'<th class="clm_checkbox"><input type="checkbox" class="selectAll" /></th>'
		+			genThead(obj.columns)
