...
ffzx.init = {
	...
	dateInput: function(obj){
		...
		var _method = ('showTime' in obj && obj.showTime == true) ? 'datetimepicker' : 'datepicker';
		var _opt_hideCalendar = {
			// Ref: http://stackoverflow.com/questions/2208480/jquery-ui-datepicker-to-show-month-year-only
			currentText: '当月',
			dateFormat: 'yy-mm',
			beforeShow: function(input, inst){
				var datestr = $(input).val();
				var dateSet = new Date();
				inst.dpDiv.addClass('month_year_datepicker hideDate');

                if (datestr.length > 0) {
                	var dateArr = datestr.split('-');
                    var year = dateArr[0];
                    var month = dateArr[1];
                    dateSet = new Date(year, month-1, 1);                
                }
                $(input)[_method]('option', 'defaultDate', dateSet);
                $(input)[_method]('setDate', dateSet);
			},
			onClose: function(dateText, inst){
				var isDonePressed = function(){
                    return (inst.dpDiv.html().indexOf('ui-datepicker-close ui-state-default ui-priority-primary ui-corner-all ui-state-hover') > -1);
                }

                if (isDonePressed()){
                    var month = inst.dpDiv.find(".ui-datepicker-month :selected").val();
                    var year = inst.dpDiv.find(".ui-datepicker-year :selected").val();
                    $(this)[_method]('setDate', new Date(year, month, 1)).trigger('change').focusout();                    
                }

				setTimeout(function(){
					inst.dpDiv.removeClass('hideDate');
				}, 300);			
			}
		};
		...
		// Only show Year & Month
		if ('showDate' in obj && obj.showDate == false) {
			$.extend(opt, _opt_hideCalendar);
		}
...
ffzx.util = {
	...
	adjustInput: function(){
      ...
      var $colRight = $block.find('>div');
			...
			if ($colRight.hasClass('f7')) {
				$colRight.find('.selectBtn').before('<i class="fa fa-times-circle clear-input" title="清空"></i>');
        $colRight.find('.clear-input').click(function(e){
          e.stopPropagation();
          $(this).closest('.f7').find(':text, :hidden').val('');
          return false;
        });
			}
			...
			$colRight.width(wBlock - wLabel - offset).addClass('pull-right');
